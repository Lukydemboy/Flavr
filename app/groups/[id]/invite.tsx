...
export default function GroupInviteScreen() {
  ...
  useEffect(() => {
    if (users && group && !isLoadingGroup) {
      setFilteredUsers(
        users.content
          .filter(user => !selectedUsers.some(selected => isEqual(selected, user)))
          .filter(user => !group.members.some(member => isEqual(member, user))),
      );
    }
      ...
      <View className="bg-white p-4 rounded-xl flex flex-row flex-wrap gap-2 mt-4">
        ...
        {selectedUsers.map(user => (
          ...
          >
            ...
            <StyledText className="text-gray-500 text-sm">{user.username}</StyledText>
            <Pressable
