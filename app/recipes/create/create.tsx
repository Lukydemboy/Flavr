...
export default function CreateRecipeScreen() {
  ...
  const [instructions, setInstructions] = useState<
    { number: number; instruction: string }[]
  >([]);
    ...
    <Page>
      {step === "General" && (
        <>
          ...
          <StyledText className="text-sm mb-4 text-slate-500">
            Give some information about your recipe, the more information you
            give the easier it is to find for others!
          </StyledText>
      ...
      {step === "Ingredients" && (
        <>
          ...
          <StyledText className="mb-4 text-slate-500">
            You can&lsquo;t cook without ingredients!
          </StyledText>
          ...
          <ScrollView contentContainerClassName="bg-white rounded-xl bg-white p-4 grow mb-4 border-2 border-slate-200">
            {ingredients.map((ingredient, index) => (
              <View
                key={index}
                className="relative flex flex-row gap-x-1 mb-2 bg-background p-4 rounded-xl"
              >
                <StyledText className="text-slate-700">
                  {ingredient.value}
                </StyledText>
        ...
        <>
          ...
          <form.Field name="instructions">
            {(field) => (
              <InputField
                ...
                value={field.state.value}
                onChangeText={(text) =>
                  form.setFieldValue("instructions", text)
                }
                ...
                onSubmitEditing={(value) => {
                  setInstructions([
                    ...instructions,
                    { number: instructions.length + 1, instruction: value },
                  ]);
          ...
          <ScrollView contentContainerClassName="bg-white rounded-xl bg-white p-4 grow mb-4 border-2 border-slate-200">
            {instructions.map((instruction, index) => (
              <View
                key={index}
                className="relative p-4 bg-slate-100 rounded-xl pl-9 mb-4"
              >
                ...
                <StyledText className="leading-normal">
